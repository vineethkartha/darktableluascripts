...
local function generate_tags_with_ai(image)
  local PYTHON = "python.exe"
  local SCRIPT = "\"C:\\Users\\Admin\\AppData\\Local\\darktable\\lua\\myscripts\\getTagsFromAI.py\""
...
-- Raw paths (no quotes for Lua functions)
  ...
  local tags, caption, writeup = parse_ai_file(RESULTFILE_PATH)
  return tags, caption, writeup
end
...
local function generate_ai_tags_and_attach(images)
  ...
  for index, image in ipairs(images) do
    local tags, caption, writeup = generate_tags_with_ai(image)
...
dt.register_event(EVENT_NAME, -- event name (unique id)
                  ...
                  function(event, shortcut)
                        generate_ai_tags_and_attach(dt.gui.selection())
                  end, _("attach tags to the images") -- label shown in Shortcuts prefs
)
