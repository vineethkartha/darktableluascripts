...
local MODULE_NAME = "ExportFileDemo"  -- make sure this is unique, no spaces, no special characters   
local EVENT_NAME = "exportfiledemo"   -- must be unique for this script
...
script_data.metadata = {
  name = "Export File Demo",
  purpose = "show how to export to jpeg",
...
local function export_image(images)
    if #images ~= 1 then
    ...
    for _, img in ipairs(images) do
        ...
        dt.print("Exporting to: " .. temp_file)
        dt.print_log("Exported to: " .. temp_file)
  end
end

export_image(dt.gui.selection())
