...
local MODULE_NAME = "Trial Script"  -- make sure this is unique, no spaces, no special characters   
local EVENT_NAME = "trialscript"   -- must be unique for this script
...
script_data.metadata = {
  name = "A  script to try things",
  purpose = "trial script",
...
local function generate_tags_title_description_with_ai(image)
  ...
  local prompt = "You are an expert in image recognition and tagging. Generate a catchy title and generate single word relevant tags. Format the output as: TAGS: tag1, tag2, tag3 CAPTION: A catchy title WRITEUP: A brief writeup about the image. "
  local LLM = "gemma3:4b"
...
local function pass_single_image(images)
    if #images ~= 1 then
...
pass_single_image(dt.gui.selection())
...
dt.register_event(
  ...
  _("Show name123")  -- label shown in Shortcuts prefs
)
